...
colnames(se)  <- fix_ids(colnames(se))
colnames(otu) <- fix_ids(colnames(otu))
rownames(meta) <- fix_ids(rownames(meta))

# keep colData aligned
...
# reorder + strict check
emb <- emb[colnames(mae[["view_16S"]]), , drop = FALSE]
stopifnot(identical(rownames(emb), colnames(mae[["view_16S"]])))
