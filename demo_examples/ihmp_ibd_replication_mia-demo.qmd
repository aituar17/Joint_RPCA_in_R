...
fix_ids <- function(ids) sub("^X(?=\\d)", "", ids, perl = TRUE)

colnames(se) <- fix_ids(as.character(colnames(se)))

colnames(otu) <- fix_ids(as.character(colnames(otu)))
rownames(meta) <- fix_ids(as.character(rownames(meta)))

if ("sample_id" %in% names(meta)) meta$sample_id <- fix_ids(as.character(meta$sample_id))

prefix_ids <- function(ids) paste0("S", as.character(ids))

colnames(se)  <- prefix_ids(colnames(se))
colnames(otu) <- prefix_ids(colnames(otu))
rownames(meta) <- prefix_ids(rownames(meta))
colData(se)$Group <- meta$Group 

# sanity
stopifnot(identical(colnames(se), colnames(otu)))
stopifnot(setequal(colnames(se), rownames(meta)))
...
set.seed(42)
# --- run engine directly (bypasses reducedDim storing bug in getJointRPCA) ---
...
# store full result on MAE
metadata(mae)$JointRPCA <- list(JointRPCA = fit)

# store embedding manually 
emb <- as.matrix(fit$ord_res$samples)

rownames(emb) <- trimws(as.character(rownames(emb)))

emb <- emb[colnames(mae[["view_16S"]]), , drop = FALSE]

reducedDim(mae[["view_16S"]], "JointRPCA") <- emb
...
#if ord_res$features is a list (multi-view API), pick the 16S view
if (is.list(fit$ord_res$features)) {
    V_16S <- as.data.frame(fit$ord_res$features[["view_16S"]])
} else {
    V_16S <- as.data.frame(fit$ord_res$features)
}
...
#variance explained
subtitle_text <- NULL
if (!is.null(fit$ord_res$proportion.explained)) {
    prop <- fit$ord_res$proportion.explained
