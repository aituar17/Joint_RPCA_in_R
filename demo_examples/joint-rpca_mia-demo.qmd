...
# Store full result on the MAE (exactly like getJointRPCA() would)
if (is.null(metadata(HintikkaXOData)$JointRPCA)) {
  metadata(HintikkaXOData)$JointRPCA <- list()
}
metadata(HintikkaXOData)$JointRPCA[["JointRPCA"]] <- res
...
# Align rows to the experiment colnames
exp_cols <- colnames(HintikkaXOData[["microbiota"]])
stopifnot(setequal(rownames(emb), exp_cols))
emb <- emb[exp_cols, , drop = FALSE]

reducedDim(HintikkaXOData[["microbiota"]], "JointRPCA") <- emb

# Retrieve results (your desired API)
result <- metadata(HintikkaXOData)$JointRPCA[["JointRPCA"]]
names(result)

# Quick sanity checks
stopifnot(!is.null(result$rclr_tables))
stopifnot(ncol(result$ord_res$samples) >= 2)
