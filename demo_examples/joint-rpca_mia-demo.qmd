...
# Choose one experiment to run Joint-RPCA on
stopifnot("microbiota" %in% names(experiments(HintikkaXOData)))
xexp <- HintikkaXOData[["microbiota"]]

set.seed(42)

xexp <- mia::getJointRPCA(
  x = xexp,
  name = "JointRPCA",
  transform = "rclr",
  n.components = 3,
...
# Put the modified experiment back into the MAE (keeps everything together)
HintikkaXOData[["microbiota"]] <- xexp

# Retrieve results (your desired API, now stored on the experiment)
result <- metadata(HintikkaXOData[["microbiota"]])$JointRPCA[["JointRPCA"]]
names(result)

emb <- reducedDim(HintikkaXOData[["microbiota"]], "JointRPCA")
dim(emb)
head(emb)
