...
run_joint_rpca_engine <- function(x_mae, ...) {
  ...
  # 2) Fallback: internal engine (unexported)
  f <- NULL
  ...
  # IMPORTANT: do NOT pass transform again here; it's already in ...
  res <- f(x = x_mae, ...)
...
ggplot(results_long, aes(x = reorder(Method, Score), y = Score)) +
  ...
  labs(x = NULL, y = "Score",
       title = paste0("Classification benchmarks (", target_label, ", ", safe_k, "-fold CV)")) +
