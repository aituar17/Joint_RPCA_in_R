...
bench_tbl <- rbind(
    cbind(model = "Joint (MGX+MTX)", t(extract_values(res_joint))),
    cbind(model = "MGX only",         t(extract_values(res_mgx))),
    cbind(model = "MTX only",         t(extract_values(res_mtx)))
) %>% as.data.frame()
...
pc1_tr <- fit_tr$ord_res$samples[, 1]
pc1_te <- fit_te$ord_res$samples[, 1]
...
MGX_h <- compute_hellinger(X_mgx)
MTX_h <- compute_hellinger(X_mtx)
V_h   <- rbind(MGX_h, MTX_h)   
stopifnot(ncol(V_h) == length(sample_ids))

se_nmf <- SummarizedExperiment::SummarizedExperiment(
    assays = list(counts = V_h)
)
...
if (requireNamespace("NMF", quietly = TRUE)) {
    ...
    # Sample scores (samples x k)
    S <- SummarizedExperiment::reducedDim(se_nmf2, "NMF")
    nmf_scores <- make_scores_df(S, rownames(S))
    cmp_nmf <- eval_wrapper(nmf_scores, sprintf("NMF (Hellinger, k=%d)", ncol(S)))

} else {
    message("[NMF] skipped: install the NMF package to enable mia::addNMF().")
}
