...
bench_tbl <- rbind(
    cbind(model = "Joint (MGX+MTX)", t(mia:::.extract_values(res_joint))),
    cbind(model = "MGX only",         t(mia:::.extract_values(res_mgx))),
    cbind(model = "MTX only",         t(mia:::.extract_values(res_mtx)))
) %>% as.data.frame()
...
MGX_h <- mia:::.compute_hellinger(X_mgx)
MTX_h <- mia:::.compute_hellinger(X_mtx)
