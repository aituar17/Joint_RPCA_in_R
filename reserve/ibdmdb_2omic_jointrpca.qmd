...
#and metadata
suppressWarnings(try(data(ibdmdb_meta_demo), silent = TRUE))

#use the prepared MultiAssayExperiment directly
mae_2 <- mae2

#sanity check & report
stopifnot(inherits(mae_2, "MultiAssayExperiment"))
msg <- sprintf("MGX samples: %d | MTX samples: %d | Shared: %d",
               ncol(experiments(mae_2)[["MGX"]]),
               ncol(experiments(mae_2)[["MTX"]]),
               length(intersect(colnames(experiments(mae_2)[["MGX"]]),
                                colnames(experiments(mae_2)[["MTX"]]))))
message(msg)

#choose a safe rank k from per-view min dims
dims <- vapply(experiments(mae_2), function(se) min(nrow(assay(se)), ncol(assay(se))), numeric(1))
k <- max(1L, min(3L, dims))
set.seed(42)
