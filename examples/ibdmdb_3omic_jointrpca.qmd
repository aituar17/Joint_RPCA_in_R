...
eval_scores <- function(scores_df) {
  ...
  if (length(axes) >= 2) {
    perm_df <- stats::na.omit(scores_df[, c("Group", axes)])
    if (is.factor(perm_df$Group) && nlevels(perm_df$Group) >= 2 && all(table(perm_df$Group) >= 5)) {
      perm <- safe(vegan::adonis2(as.matrix(perm_df[, axes]) ~ Group, data = perm_df, method = "euclidean"))
      if (!is.null(perm)) {
...
#enforce identical sample set/order
...
#IBD vs non-IBD grouping from metadata
