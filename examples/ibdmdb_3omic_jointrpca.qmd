...
#paths
f_mgx <- "examples/data_ibdmdb_raw/taxonomic_profiles_mgx.tsv"   #metagenomics
f_mtx <- "examples/data_ibdmdb_raw/ecs_relab.tsv"                #metatranscriptomics
f_vrx <- "examples/data_ibdmdb_raw/taxonomic_profiles_vrx.tsv"   #viromics
f_meta <- "examples/hmp2_metadata_2018-08-20.csv"                #HMP2/IBDMDB master metadata
...
#sparsity filtering: keep features present in ≥5% of samples
min_freq <- ceiling(0.05 * ncol(X_mgx))
keep_mgx <- rowSums(X_mgx > 0) >= min_freq
keep_mtx <- rowSums(X_mtx > 0) >= min_freq
keep_vrx <- rowSums(X_vrx > 0) >= min_freq
X_mgx <- X_mgx[keep_mgx, , drop = FALSE]
X_mtx <- X_mtx[keep_mtx, , drop = FALSE]
X_vrx <- X_vrx[keep_vrx, , drop = FALSE]

cat("\nRemaining features after 5% freq filter:\n",
"MGX:", nrow(X_mgx), "\n",
"MTX:", nrow(X_mtx), "\n",
"VRX:", nrow(X_vrx), "\n")
