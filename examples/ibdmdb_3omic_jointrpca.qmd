...
if (need_fix) {
  # read names only, keep raw, then normalize sample columns
  nm_mgx0 <- names(data.table::fread(f_mgx_al, nrows = 0, header = TRUE, check.names = FALSE))
  nm_mtx0 <- names(data.table::fread(f_mtx_al, nrows = 0, header = TRUE, check.names = FALSE))

  nm_mgx1 <- nm_mgx0
  nm_mtx1 <- nm_mtx0
  if (length(nm_mgx1) >= 2) nm_mgx1[-1] <- norm_keys(nm_mgx1[-1])
  if (length(nm_mtx1) >= 2) nm_mtx1[-1] <- norm_keys(nm_mtx1[-1])

  mgx_cols <- setdiff(nm_mgx1, nm_mgx1[1])
  mtx_cols <- setdiff(nm_mtx1, nm_mtx1[1])

  cat("\n[need_fix] After normalization: |MGX| =", length(mgx_cols),
      "|MTX| =", length(mtx_cols),
      "|MGX∩MTX| =", length(intersect(mgx_cols, mtx_cols)), "\n")
...
# Compact metrics if you want them as a list
metrics <- list(
  wilcox_PC1_p = if (!inherits(w1,"try-error")) w1$p.value else NA_real_,
  wilcox_PC2_p = if (!inherits(w2,"try-error")) w2$p.value else NA_real_
)
print(metrics)
