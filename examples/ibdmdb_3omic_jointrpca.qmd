...
join_with_labels <- function(meta, sample_ids) {
  if (is.null(meta) || !nrow(meta)) return(data.frame(sample_id = sample_ids, Group = factor(NA, levels=c("IBD","non-IBD"))))

  m0 <- meta
  names(m0) <- tolower(trimws(names(m0)))
  # likely ID columns in HMP2/IBDMDB
  id_cands <- c("external id","external.id","external_id",
  ...
  # pick the id column with the largest overlap
  best <- NULL; best_hits <- -1
  for (c in id_cands) {
    hits <- sum(tolower(trimws(as.character(m0[[c]]))) %in% tolower(sample_ids))
    if (hits > best_hits) { best <- c; best_hits <- hits }
  }

  # pick a diagnosis-like column
  ...
  if (!length(diag_cands)) return(data.frame(sample_id = sample_ids, Group = factor(NA, levels=c("IBD","non-IBD"))))

  # derive labels
  x <- tolower(trimws(as.character(m0[[diag_cands[1]]])))
  grp <- ifelse(grepl("\\b(uc|cd|ibd)\\b", x), "IBD",
                ifelse(grepl("^\\s*non", x) | grepl("healthy|control", x), "non-IBD", NA_character_))
  j <- data.frame(sample_id = tolower(trimws(as.character(m0[[best]]))), Group = grp, stringsAsFactors = FALSE)
  j <- unique(j[!is.na(j$sample_id), ])
  out <- dplyr::left_join(
    data.frame(sample_id = sample_ids, stringsAsFactors = FALSE),
    j, by = c("sample_id" = "sample_id")
  )
