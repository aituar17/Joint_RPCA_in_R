...
#before building SEs, lock row orders as attributes for sanity checks
attr(X_16s, "rn") <- rownames(X_16s)
attr(X_mgx, "rn") <- rownames(X_mgx)
attr(X_mtx, "rn") <- rownames(X_mtx)

stopifnot(identical(colnames(X_16s), colnames(X_mgx)),
          identical(colnames(X_mgx), colnames(X_mtx)))
...
mae_3 <- MultiAssayExperiment::MultiAssayExperiment(
  experiments = list(S16 = se_S16, MGX = se_MGX, MTX = se_MTX)
)
mae_3 <- MultiAssayExperiment::intersectColumns(mae_3)

stopifnot(identical(rownames(SummarizedExperiment::assay(mae_3[["S16"]], "counts")), attr(X_16s, "rn")),
          identical(rownames(SummarizedExperiment::assay(mae_3[["MGX"]], "counts")), attr(X_mgx, "rn")),
