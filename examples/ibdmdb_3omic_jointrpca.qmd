...
shared_3 <- unique(shared_3)
shared_3 <- shared_3[nchar(shared_3) > 0]
stopifnot(length(shared_3) >= 20)

#subset each matrix to the same ordered set of sample IDs
shared_3 <- sort(shared_3)
X_mgx <- mat_mgx[, shared_3, drop = FALSE]
X_mtx <- mat_mtx[, shared_3, drop = FALSE]
X_vrx <- mat_vrx[, shared_3, drop = FALSE]

#sanity checks
stopifnot(
  identical(colnames(X_mgx), shared_3),
  identical(colnames(X_mtx), shared_3),
  identical(colnames(X_vrx), shared_3),
  !any(duplicated(shared_3))
)

# Per-modality sparsity filtering
...
if (nrow(X_vrx) < min_vrx_features) {
  message(sprintf("[VRX] Only %d features after %.1f%% prev. Relaxing to 0.5%%…",
                  nrow(X_vrx), 100 * prev_vrx / n_samp))
  prev_vrx <- max(1L, ceiling(0.005 * n_samp))
  keep_vrx <- rowSums(X_vrx > 0) >= prev_vrx
  X_vrx <- X_vrx[keep_vrx, , drop = FALSE]
  if (nrow(X_vrx) < min_vrx_features) {
...
rclr <- function(X, pseudocount = 0.5) {
  #X: features x samples (numeric), nonnegative
  Xp <- X + pseudocount
  LX <- log(Xp)
  #robust center per sample (column medians)
  cm <- if (requireNamespace("matrixStats", quietly = TRUE)) {
    matrixStats::colMedians(LX, na.rm = FALSE, useNames = FALSE)
  } else {
    apply(LX, 2, stats::median, na.rm = FALSE)  
  }
  sweep(LX, 2, cm, FUN = "-")  
}

#apply rCLR to each modality
Xr_mgx <- rclr(X_mgx, pseudocount = 0.5)
Xr_mtx <- rclr(X_mtx, pseudocount = 0.5)
Xr_vrx <- rclr(X_vrx, pseudocount = 0.5)

#final sanity: identical sample IDs across all transformed matrices
stopifnot(
  identical(colnames(Xr_mgx), colnames(Xr_mtx)),
  identical(colnames(Xr_mgx), colnames(Xr_vrx))
)

# Build a MultiAssayExperiment and run universal Joint-RPCA

#make sure columns are in the same order and numeric
stopifnot(
  identical(colnames(X_mgx), shared_3),
  identical(colnames(X_mtx), shared_3),
  identical(colnames(X_vrx), shared_3)
)
X_mgx <- as.matrix(X_mgx); storage.mode(X_mgx) <- "numeric"
X_mtx <- as.matrix(X_mtx); storage.mode(X_mtx) <- "numeric"
X_vrx <- as.matrix(X_vrx); storage.mode(X_vrx) <- "numeric"

#common colData indexed by the shared sample IDs
cd <- S4Vectors::DataFrame(row.names = shared_3)

#make feature names unique + numeric, drop NA/Inf 
...
X_vrx <- sanitize_matrix(dedup_rownames(X_vrx))

#identical sample order across assays
stopifnot(
  identical(colnames(X_mgx), shared_3),
  identical(colnames(X_mtx), shared_3),
  identical(colnames(X_vrx), shared_3)
...
#one SummarizedExperiment per modality (rows = features, cols = samples)
...
se_vrx <- SummarizedExperiment::SummarizedExperiment(
  assays  = list(counts = X_vrx),
  colData = cd
)

#multiAssayExperiment with identical sample names across assays
mae_3 <- MultiAssayExperiment::MultiAssayExperiment(
  experiments = list(MGX = se_mgx, MTX = se_mtx, VRX = se_vrx)
)
#ensure column intersection
mae_3 <- MultiAssayExperiment::intersectColumns(mae_3)

#choose a safe rank across all assays
per_view_min_dim <- sapply(list(X_mgx, X_mtx, X_vrx), function(m) min(nrow(m), ncol(m)))
k_max <- min(per_view_min_dim)                 #largest rank allowed by all views
if (k_max < 2) stop(sprintf(
  "Not enough features/samples for factorization (min per-view min(nrow,ncol) = %d).",
  k_max
))
k <- min(3L, k_max)                            #request up to 3, but never exceeding k_max
message(sprintf("[rank] per-view min dims = %s; using n.components = %d",
                paste(per_view_min_dim, collapse = ", "), k))

#run Joint-RPCA with internal rCLR and without pre-transforming input
set.seed(42)
...
str(fit3, max.level = 1)

#sample scores (U) and per-modality loadings (V)
U <- as.data.frame(fit3$ord.res$samples)
colnames(U) <- paste0("V", seq_len(ncol(U)))
U$sample_id <- rownames(U)

V_mgx <- as.data.frame(fit3$ord.res$features$MGX)
V_mtx <- as.data.frame(fit3$ord.res$features$MTX)
V_vrx <- as.data.frame(fit3$ord.res$features$VRX)

#variance explained
if (!is.null(fit3$ord.res$proportion.explained)) {
  prop <- fit3$ord.res$proportion.explained
  message(sprintf("Explained variance: PC1 %.1f%%, PC2 %.1f%%, PC3 %.1f%%",
...
print(
  ggplot(U, aes(V1, V2)) +
    ...
    labs(title = "Joint-RPCA — MGX + MTX + VRX", x = "PC1", y = "PC2") +
    theme_minimal()
)
