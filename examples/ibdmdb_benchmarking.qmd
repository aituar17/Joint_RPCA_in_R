...
# Standard PCA / CCA / MOFA2 / mixOmics 
...
#use mixOmics::rcc if available (regularized CCA); else try CCA::cc
cmp_cca <- NULL
if (requireNamespace("mixOmics", quietly = TRUE)) {
  X1 <- t(zscore_rows(MGX_clr))
  X2 <- t(zscore_rows(MTX_clr))
  #regularization heuristics
  cca_fit <- try(mixOmics::rcc(X1, X2, ncomp = k0, lambda1 = 0.1, lambda2 = 0.1), silent = TRUE)
  if (!inherits(cca_fit, "try-error")) {
    #combine block variates (averaging standardized block scores)
    vx <- as.matrix(cca_fit$variates$X)
    vy <- as.matrix(cca_fit$variates$Y)
    #keep common number of components across both blocks
    kk <- min(ncol(vx), ncol(vy), k0)
    vx <- vx[, seq_len(kk), drop = FALSE]
    vy <- vy[, seq_len(kk), drop = FALSE]
    #z-standardize each variate then average
    z <- scale(vx); z2 <- scale(vy)
    cca_combined <- (z + z2) / 2
    cca_scores <- make_scores_df(cca_combined, rownames(cca_combined))
    cmp_cca <- eval_wrapper(cca_scores, sprintf("CCA (mixOmics rcc, k=%d)", kk))
  } else {
    message("[CCA] mixOmics::rcc failed: ", as.character(cca_fit))
  }
} else if (requireNamespace("CCA", quietly = TRUE)) {
  X1 <- t(zscore_rows(MGX_clr))
  X2 <- t(zscore_rows(MTX_clr))
  cca_fit <- try(CCA::cc(X1, X2), silent = TRUE)
  if (!inherits(cca_fit, "try-error")) {
    kk <- min(k0, ncol(cca_fit$scores$xs), ncol(cca_fit$scores$ys))
    vx <- cca_fit$scores$xs[, seq_len(kk), drop = FALSE]
    vy <- cca_fit$scores$ys[, seq_len(kk), drop = FALSE]
    cca_combined <- (scale(vx) + scale(vy)) / 2
    cca_scores <- make_scores_df(cca_combined, rownames(cca_combined))
    cmp_cca <- eval_wrapper(cca_scores, sprintf("CCA (CCA::cc, k=%d)", kk))
  } else {
    message("[CCA] CCA::cc failed: ", as.character(cca_fit))
  }
} else {
  message("[CCA] skipped: install mixOmics or CCA.")
}
...
if (requireNamespace("MOFA2", quietly = TRUE)) {
  ...
  X2 <- t(zscore_rows(MTX_clr))
  #build MOFA object
  mofa_data <- list(MGX = X1, MTX = X2)
  MO <- try(MOFA2::create_mofa(mofa_data), silent = TRUE)
  if (!inherits(MO, "try-error")) {
    data_opts <- MOFA2::get_default_data_options(MO)
    model_opts <- MOFA2::get_default_model_options(MO); model_opts$num_factors <- k0
    train_opts <- MOFA2::get_default_training_options(MO); train_opts$seed <- 42; train_opts$maxiter <- 1000
    MO <- try(MOFA2::prepare_mofa(MO, data_opts, model_opts, train_opts), silent = TRUE)
    MO <- try(MOFA2::run_mofa(MO, use_basilisk = TRUE), silent = TRUE)
    if (!inherits(MO, "try-error")) {
      Z <- MOFA2::get_factors(MO, factors = paste0("Factor", seq_len(k0)), as.data.frame = FALSE)$group1
      mofa_scores <- make_scores_df(as.matrix(Z), rownames(Z))
      cmp_mofa <- eval_wrapper(mofa_scores, sprintf("MOFA2 (k=%d)", ncol(Z)))
    } else {
      message("[MOFA2] training failed; skipped.")
    }
  } else {
    message("[MOFA2] create_mofa failed; skipped.")
  }
} else {
  message("[MOFA2] skipped: install MOFA2.")
}

#4) mixOmics
cmp_block_splsda <- NULL
if (requireNamespace("mixOmics", quietly = TRUE)) {
  #use only labeled samples
  lab <- grp_df$Group
  keep <- which(!is.na(lab))
  if (length(keep) >= 20 && length(unique(lab[keep])) >= 2) {
    X1 <- t(zscore_rows(MGX_clr)) [keep, , drop = FALSE]
    X2 <- t(zscore_rows(MTX_clr)) [keep, , drop = FALSE]
    Y  <- droplevels(lab[keep])
    keepX <- list(MGX = rep(min(25, ncol(X1)), k0),
                  MTX = rep(min(25, ncol(X2)), k0))
    design <- "full"
    bsda <- try(mixOmics::block.splsda(
      X = list(MGX = X1, MTX = X2), Y = Y, ncomp = k0, keepX = keepX, design = design
    ), silent = TRUE)
    if (!inherits(bsda, "try-error")) {
      #combine block variates (samples x k) by averaging standardized block scores
      vx <- as.matrix(bsda$variates$MGX)
      vy <- as.matrix(bsda$variates$MTX)
      kk <- min(ncol(vx), ncol(vy), k0)
      comb <- (scale(vx[, seq_len(kk), drop = FALSE]) + scale(vy[, seq_len(kk), drop = FALSE]))/2
      #build scores for ALL samples: NA for unlabeled (since model trained on labeled only)
      full <- matrix(NA_real_, nrow = length(sample_ids), ncol = kk,
                     dimnames = list(sample_ids, paste0("V", seq_len(kk))))
      full[rownames(comb), ] <- comb
      bsda_scores <- make_scores_df(full, sample_ids)
      cmp_block_splsda <- eval_wrapper(bsda_scores, sprintf("mixOmics block.splsda (k=%d)", kk))
    } else {
      message("[block.splsda] mixOmics failed: ", as.character(bsda))
    }
  } else {
    message("[block.splsda] skipped: not enough labeled samples.")
  }
} else {
  message("[block.splsda] skipped: install mixOmics.")
}
...
comparators <- Filter(Negate(is.null),
  list(cmp_pca = cmp_pca,
       cmp_cca = cmp_cca,
       cmp_mofa = cmp_mofa,
       cmp_block_splsda = cmp_block_splsda)
)
...
if (length(comparators)) {
  ...
  knitr::kable(comp_tbl, digits = 3, caption = "Comparators: PCA / CCA / MOFA2 / mixOmics")
