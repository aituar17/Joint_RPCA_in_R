...
eval_scores <- function(scores_df) {
  ...
  if (nrow(rf_df) && is.factor(rf_df$Group) && nlevels(rf_df$Group) >= 2) {
    ...
    set.seed(42)
    rf_prob <- ranger::ranger(Group ~ ., data = rf_df, num.trees = 200,
...
perm_test_scores <- function(scores_df, nperm = 200) {
  #observed AUROC from already-fitted scores
  obs <- eval_scores(scores_df)$AUROC
  #shuffle only labels
  null <- replicate(nperm, {
    tmp <- scores_df
    tmp$Group <- sample(tmp$Group)
    eval_scores(tmp)$AUROC
  })
  list(observed = obs, null = null, p_emp = mean(null >= obs, na.rm = TRUE))
}

pt <- perm_test_scores(res_joint$scores, nperm = 100)
