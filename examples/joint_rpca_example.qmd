...
fold_tbl <- dplyr::bind_rows(
  ...
  get_fold_metrics(mofa_top3)           |> dplyr::mutate(Method="MOFA+ (top3)")
)
...
#paired Wilcoxon between Joint‑RPCA and strongest baselines
pair_test <- function(metric, m1, m2){
  x <- fold_tbl |> filter(Method==m1) |> pull({{metric}})
  y <- fold_tbl |> filter(Method==m2) |> pull({{metric}})
  idx <- is.finite(x)&is.finite(y)
  tibble::tibble(Compare=paste(m1,"vs",m2), Metric=deparse(substitute(metric)),
                 Diff=mean(x[idx]-y[idx]), Wilcoxon_p=wilcox.test(x[idx],y[idx],paired=TRUE,exact=FALSE)$p.value)
}
bind_rows(
  pair_test(MacroAUC, "Joint‑RPCA", "Per‑layer RPCA"),
  pair_test(MacroAUC, "Joint‑RPCA", "Per‑layer PCA"),
  pair_test(Accuracy, "Joint‑RPCA", "Per‑layer RPCA"),
  pair_test(Accuracy, "Joint‑RPCA", "Per‑layer PCA")
) |> print()
