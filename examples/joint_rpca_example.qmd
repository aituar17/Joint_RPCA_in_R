...
evaluate_model_cv <- function(features, labels, folds = 5) {
  ...
  for (i in seq_along(folds_idx)) {
    ...
    pred_class <- randomForest::predict(rf_model, features[test_idx, ])
    acc <- mean(pred_class == labels[test_idx])
    
    pred_probs <- randomForest::predict(rf_model, features[test_idx, ], type = "prob")
    auc <- pROC::multiclass.roc(labels[test_idx], pred_probs)$auc
