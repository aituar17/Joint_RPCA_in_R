...
#MOFA+ latent factors
mofa_in <- lapply(result$rclr.tables, function(M) M[, common_samples, drop = FALSE])
mofa <- create_mofa(mofa_in)
mofa <- prepare_mofa(mofa)
mofa <- train_mofa(mofa, verbose = FALSE)
mofa_factors <- get_factors(mofa, factors = "all")[[1]]       
mofa_factors <- mofa_factors[rownames(features_jointRPCA), ] 
