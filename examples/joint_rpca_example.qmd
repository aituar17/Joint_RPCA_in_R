...
features_rclr_concat <- do.call(cbind, result$rclr.tables)

#prepare single omic RPCA scores as an input feature set
features_microbiota <- dataset_specific_scores[["microbiota"]]
features_metabolites <- dataset_specific_scores[["metabolites"]]
features_biomarkers <- dataset_specific_scores[["biomarkers"]]
...
res_microbiota <- evaluate_model_cv(features_microbiota, labels)
res_metabolites <- evaluate_model_cv(features_metabolites, labels)
res_biomarkers <- evaluate_model_cv(features_biomarkers, labels)
res_random <- evaluate_model_cv(features_random, labels)

#create and print a summary table
results_df <- data.frame(
  Method = c("Joint-RPCA", "Per-layer RPCA", "Raw rCLR-Transformed Features (Per Omic)", "Microbiota RPCA Scores", "Metabolites RPCA Scores", "Biomarkers RPCA Scores", "Random"),
  Accuracy = c(res_joint$accuracy, res_rpca$accuracy, res_rclr$accuracy, res_microbiota$accuracy, res_metabolites$accuracy, res_biomarkers$accuracy, res_random$accuracy),
  AUC = c(res_joint$auc, res_rpca$auc, res_rclr$auc, res_microbiota$auc, res_metabolites$auc, res_biomarkers$auc, res_random$auc)
)
