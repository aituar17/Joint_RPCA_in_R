...
# 1) drop NAs
keep_idx <- !is.na(labels)

# 2) drop classes with < 2 samples (caret::createFolds needs at least 2 per class)
tab <- table(labels[keep_idx])
keep_classes <- names(tab)[tab >= 2]
keep_idx <- keep_idx & labels %in% keep_classes

# 3) subset all features and labels to the "safe" set
labels <- droplevels(labels[keep_idx])
...
# 4) choose a safe K for CV (cannot exceed min class size)
min_class_n <- min(table(labels))
safe_k <- max(2, min(5, as.integer(min_class_n)))

evaluate_model_cv <- function(features, labels, folds = 5, ntree = 500, seed = 42) {
  set.seed(seed)
  # cap folds to the smallest class size at call-time, just in case
  folds <- max(2, min(folds, as.integer(min(table(labels)))))
  ...
  for (i in seq_along(folds_idx)) {
    ...
    # Defensive: if train fold has <2 classes (can happen in tiny data), skip this fold
    ...
    # center/scale on train only (no leakage)
    pp <- prep_train_test(Xtr, Xte)
    Xtr <- pp$Xtr; Xte <- pp$Xte

    rf <- randomForest::randomForest(x = Xtr, y = ytr, ntree = ntree)

    # Accuracy
    yhat <- predict(rf, Xte, type = "response")
    accs[i] <- mean(yhat == yte)

    # Macro AUC (guard when test fold single-class)
    probs <- predict(rf, Xte, type = "prob")
    all_lvls <- levels(labels)
    if (!all(all_lvls %in% colnames(probs))) {
      missing <- setdiff(all_lvls, colnames(probs))
      for (mm in missing) probs <- cbind(probs, setNames(rep(0, nrow(probs)), mm))
      probs <- probs[, all_lvls, drop = FALSE]
    }

    if (length(unique(yte)) < 2L) {
      # pROC can't compute AUC for single-class test fold
      aucs[i] <- NA_real_
    } else {
      aucs[i] <- tryCatch(
        as.numeric(pROC::multiclass.roc(yte, probs)$auc),
