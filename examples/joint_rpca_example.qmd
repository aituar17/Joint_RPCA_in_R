...
dataset_specific_pca_scores <- lapply(result$rclr.tables, function(tbl) {
  tbl_clean <- tbl[complete.cases(tbl) & rowSums(is.finite(tbl)) == ncol(tbl), ]
  pca <- prcomp(tbl_clean, center = TRUE, scale. = TRUE)
  pca$x[, 1:3]
})
...
results_df <- data.frame(
  Method = c("Joint-RPCA", "Per-layer RPCA", "Per-layer PCA", "Random"),
  Accuracy = c(res_joint$accuracy, res_rpca$accuracy, res_pca$accuracy, res_random$accuracy),
  AUC = c(res_joint$auc, res_rpca$auc, res_pca$auc, res_random$auc)
)
