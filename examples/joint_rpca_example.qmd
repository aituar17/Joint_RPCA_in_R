...
dir.create("examples/interop", showWarnings = FALSE)

for (i in seq_along(rclr_list)) {
  fn <- file.path("examples/interop", sprintf("view_%d_rclr.tsv", i))
  write.table(rclr_list[[i]],
              file = fn,
              sep = "\t",
              quote = FALSE,      
              row.names = TRUE,
...
write.table(data.frame(sample = common_samples),
            file = "examples/interop/samples.tsv",
            sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")

settings <- list(
  n_components = ncol(result$ord.res$samples),  
  max_iter     = 500,                            
  seed         = 42
)
...
write.table(result$ord.res$samples[common_samples, , drop = FALSE],
            file = "examples/interop/R_samplescores.tsv", sep = "\t",
            quote = FALSE, row.names = TRUE, col.names = NA, fileEncoding = "UTF-8")
