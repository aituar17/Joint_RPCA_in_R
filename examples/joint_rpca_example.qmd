...
mofa_input <- lapply(result$rclr.tables, function(tbl) {
  tbl <- tbl[, common_samples[common_samples %in% colnames(tbl)], drop = FALSE]  
  tbl <- tbl[, common_samples, drop = FALSE]  
  t(tbl) 
})
names(mofa_input) <- names(result$rclr.tables)
...
mofa <- run_mofa(mofa)
features_mofa <- get_factors(mofa, factors = "all", as.data.frame = TRUE)
features_mofa <- as.matrix(features_mofa[, -1]) 
