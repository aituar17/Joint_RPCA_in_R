...
evaluate_model <- function(features, labels) {
  ...
  pred_class <- predict(rf_model, features)
  acc <- mean(pred_class == labels)
  pred_probs <- predict(rf_model, features, type = "prob")
  multiclass_auc <- multiclass.roc(labels, pred_probs)$auc
  
  return(list(accuracy = acc, auc = multiclass_auc))
}
...
str(labels)           # Check type and levels
table(labels)         # Check class balance
summary(features_jointRPCA)  # Look for extreme values
cor(features_jointRPCA[,1], as.numeric(labels))  # Check for leakage
