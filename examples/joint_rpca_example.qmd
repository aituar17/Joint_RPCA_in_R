...
common_samples <- Reduce(intersect, c(list(rn_joint), rn_rclr))
...
dataset_specific_pca_scores <- lapply(result$rclr.tables, function(tbl) {
  ...
  keep_cols <- apply(tbl, 2, function(x) var(x, na.rm = TRUE) > 0)
  tbl <- tbl[, keep_cols, drop = FALSE]
  
  # If no columns remain, return dummy matrix
  if (ncol(tbl) == 0) {
    return(matrix(0, nrow = length(common_samples), ncol = K_pcs))
  }
