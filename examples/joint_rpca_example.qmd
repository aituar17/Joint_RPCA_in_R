...
assay_map <- lapply(HintikkaXOData, SummarizedExperiment::assayNames)
print(assay_map) 

#robust helper to fetch a counts-like matrix
prefer_assays <- c("counts", "raw", "abundance", "otu", "table", "exprs")

get_assay_matrix <- function(x) {
  an <- SummarizedExperiment::assayNames(x)
  #pick first preferred name that exists
  pick <- prefer_assays[prefer_assays %in% an]
  if (length(pick) == 0) {
    if (length(an) == 0) stop("Object has no assays.")
    pick <- an[1]  #fallback to the first assay
    message("Warning: using assay '", pick, "' because preferred names were not found.")
  }
  SummarizedExperiment::assay(x, pick[1])
}

#compute common samples safely
common_samples <- Reduce(intersect, lapply(HintikkaXOData, colnames))
if (length(common_samples) == 0) stop("No overlapping sample IDs across views.")

#build raw_list using the helper, then subset to common_samples
raw_list <- lapply(HintikkaXOData, get_assay_matrix)
...
#the existing rclr_list creation (unchanged)
