...
evaluate_model_cv <- function(features, labels, folds = 5, ntree = 500, seed = 42) {
  ...
  for (i in seq_along(folds_idx)) {
    ...
    # center/scale on train only
    pp <- prep_train_test(Xtr, Xte)
    Xtr <- pp$Xtr; Xte <- pp$Xte

    rf <- randomForest(x = Xtr, y = ytr, ntree = ntree)

    # Accuracy
    yhat <- predict(rf, Xte, type = "response")
    accs[i] <- mean(yhat == yte)

    # Macro AUC (multi-class friendly)
    # If binary, pROC handles it; if multi-class, multiclass.roc works with a matrix of probs
    probs <- predict(rf, Xte, type = "prob")
    # Guard: if any class absent in train fold, RF may miss a prob column → add zeros
    ...
    aucs[i] <- as.numeric(multiclass.roc(yte, probs)$auc)
  }
...
# ========== Run all baselines ==========
res_joint   <- evaluate_model_cv(features_jointRPCA, labels, folds = 5)
res_rpca    <- evaluate_model_cv(features_rpca_concat, labels, folds = 5)
res_pca     <- evaluate_model_cv(features_pca_concat,  labels, folds = 5)
res_concat  <- evaluate_model_cv(features_concat_pca,  labels, folds = 5)
res_random  <- evaluate_model_cv(features_random,      labels, folds = 5)
