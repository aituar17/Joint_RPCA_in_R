...
# Choose label column explicitly (you found "Fat" to be viable)
target_label <- "Fat"

# 1) Common samples across all rCLR tables ----------------
...
# 3a) Concatenated rCLR -> PCA (global PCA baseline)
...
# 3c) Per-layer RPCA -> concatenate (aligned)
...
# 3d) Joint-RPCA shared scores (aligned)
...
results_df <- tibble::tibble(
  Method   = c("Joint‑RPCA (shared scores)",
               ...
               "Raw rCLR → concat",
               "Random"),
  Accuracy = c(res_joint$accuracy, res_rpca$accuracy, res_pca$accuracy, res_concat$accuracy, res_rclr_rf$accuracy, res_random$accuracy),
  MacroAUC = c(res_joint$auc,      res_rpca$auc,      res_pca$auc,      res_concat$auc,     res_rclr_rf$auc,      res_random$auc)
