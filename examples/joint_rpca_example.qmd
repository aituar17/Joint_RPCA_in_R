...
rclr.tables.mofa <- lapply(result$rclr.tables, function(tbl) {
  tbl[, common_samples, drop = FALSE]
})
names(rclr.tables.mofa) <- names(result$rclr.tables)
mofa <- create_mofa(rclr.tables.mofa)
...
dataset_specific_scores.sub <- lapply(dataset_specific_scores, function(M) {
  M[common_samples, , drop = FALSE]
})
features_rpca_concat <- do.call(cbind, dataset_specific_scores.sub)
features_rclr_concat <- do.call(cbind, lapply(result$rclr.tables, function(tbl) t(tbl[common_samples, , drop = FALSE])))
features_rclr_concat <- features_rclr_concat[, colSums(is.na(features_rclr_concat)) == 0, drop = FALSE]
features_pca_concat <- prcomp(features_rclr_concat)$x[, 1:3, drop = FALSE]
features_mofa <- features_mofa[common_samples, , drop = FALSE]
if (!is.null(names(labels))) {
  labels <- labels[common_samples]
} else {
  rn <- rownames(colData(HintikkaXOData))
  names(labels) <- rn
  labels <- labels[common_samples]
}
...
res_mofa <- evaluate_model_cv(features_mofa, labels)
