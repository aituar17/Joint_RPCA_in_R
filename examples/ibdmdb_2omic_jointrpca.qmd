...
#иuild IBD/non-IBD labels by auto-detecting the sample-id column
meta_df <- if (file.exists(f_meta)) data.frame(data.table::fread(f_meta)) else NULL

make_groups_autodetect <- function(meta_df, sample_ids) {
  if (is.null(meta_df) || !nrow(meta_df)) {
    return(data.frame(sample_id = sample_ids, Group = factor(NA, levels = c("IBD", "non-IBD"))))
  }
  ...
  #normalize a character copy of sample ids for robust comparison
  sid <- tolower(trimws(as.character(sample_ids)))

  #compute overlap for every metadata column
  overlaps <- sapply(md, function(col) {
    x <- tolower(trimws(as.character(col)))
    sum(!is.na(x) & x %in% sid)
  })

  #take the column with the most matches (must be at least 1% or >= 10)
  best <- names(sort(overlaps, decreasing = TRUE))[1]
  if (length(best) == 0L || overlaps[[best]] < max(10, length(sid)*0.01)) {
    message("[meta] No metadata column sufficiently matches sample IDs; leaving Group = NA.")
    return(data.frame(sample_id = sample_ids, Group = factor(NA, levels = c("IBD", "non-IBD"))))
  }
  message(sprintf("[meta] Chosen sample-id column: '%s' (matches = %d)", best, overlaps[[best]]))

  #construct Group from 'diagnosis' (present in your file)
  if (!"diagnosis" %in% names(md)) {
    message("[meta] 'diagnosis' column not found; leaving Group = NA.")
    return(data.frame(sample_id = sample_ids, Group = factor(NA, levels = c("IBD", "non-IBD"))))
  }
  ...
  #restore original (non-lowered) ids for readability
...
#quick sanity check
print(head(table(grp_df$Group, useNA = "ifany")))
