...
meta_df <- if (file.exists(f_meta)) data.table::fread(f_meta, check.names = FALSE) else NULL
meta_df <- if (is.null(meta_df)) NULL else as.data.frame(meta_df, stringsAsFactors = FALSE)
grp_df <- tryCatch(
  make_group_from_meta(meta_df, U$sample_id),
  error = function(e) {
    message("[meta] Group mapping failed: ", conditionMessage(e))
    data.frame(sample_id = U$sample_id,
               Group = factor(NA_character_, levels = c("IBD", "non-IBD")),
               stringsAsFactors = FALSE)
  }
)
