...
k <- 3
set.seed(42)
fit <- jointRPCAuniversal(mae, n.components = k, max.iterations = 500,
...
if (is.list(fit$ord.res$features)) {
  V_mgx <- fit$ord.res$features$MGX %||% fit$ord.res$features$view_MGX
  V_mtx <- fit$ord.res$features$MTX %||% fit$ord.res$features$view_MTX
} else {
  message("[warn] fit$ord.res$features is not a list; treating as a single shared matrix")
  V_mgx <- as.data.frame(fit$ord.res$features)
  V_mtx <- NULL
}
...
if(!is.null(V_mgx)){
  ...
  lr_df <- dplyr::left_join(lr_df, plot_df[, c("sample_id", "Group")], by = "sample_id")
  print(
