...
make_group_from_meta <- function(meta_df, sample_ids) {
  ...
  cn_raw <- names(meta_df)
  cn_std <- tolower(trimws(cn_raw))
  names(meta_df) <- cn_std  # keep data, just fix the names

  #candidate sample-id columns (lowercase, trimmed)
  sample_cols <- tolower(trimws(c(
    "External.ID","External ID","external_id","Sample.ID","Sample ID",
    "sample_id","MGX.SampleID","MTX.SampleID","wr id","wr_id","wrid"
  )))
  sc <- intersect(sample_cols, names(meta_df))
  if (!length(sc)) return(out)

  #pick the first sample-id column with enough hits
  chosen <- NULL
  for (cand in sc) {
    x <- meta_df[[cand]]
    x <- as.character(x)
  ...
  #reorder to put chosen id first and rename it 'sample_id'
  md <- meta_df[, c(chosen, setdiff(names(meta_df), chosen)), drop = FALSE]
  names(md)[1] <- "sample_id"

  #try multiple diagnosis-ish columns (lowercase)
  diag_cols <- tolower(trimws(c(
  ...
  if (!length(dc)) return(out)

  #build Group = IBD / non-IBD from the first usable diagnosis-like column
  for (col in dc) {
    x <- md[[col]]
    x <- as.character(x)             
    if (!length(x)) next             

    #make a same-length vector of NAs, then fill IBD/non-IBD
    ...
    #only accept if we have enough labels and at least 2 classes
    if (sum(!is.na(grp)) >= max(10, length(sample_ids) * 0.05) &&
        length(unique(na.omit(grp))) >= 2) {
      md$Group <- grp  

      #join back to the requested sample_ids (force dplyr join)
      joined <- dplyr::left_join(out, 
  ...
  #if no diagnosis column produced adequate labels, return NA groups
  out
}
